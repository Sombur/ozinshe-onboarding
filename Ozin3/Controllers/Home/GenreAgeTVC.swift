...
class GenreAgeTVC: UITableViewCell {
    ...
    let collectionView = {
        ...
        cv.register(GenreAgeCVC.self, forCellWithReuseIdentifier: "GenreCell")
...
extension GenreAgeTVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "GenreCell", for: indexPath) as! GenreAgeCVC
        
        if mainMovie.cellType == .ageCategory {
            cell.image1.sd_setImage(with: URL(string: mainMovie.categoryAges[indexPath.row].link), placeholderImage: nil)
            cell.label1.text = mainMovie.categoryAges[indexPath.row].name
        } else {
            cell.image1.sd_setImage(with: URL(string: mainMovie.genres[indexPath.row].link), placeholderImage: nil)
            cell.label1.text = mainMovie.genres[indexPath.row].name
        }
        return cell
    }
